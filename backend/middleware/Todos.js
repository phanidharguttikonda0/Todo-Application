...
async function authorizationCheck(req, res, next) {
  try {
    ...
    if (result.expiry < Date.now())
      return res
        .status(403)
        .json({ message: "Authorization was expired", value: false });
...
async function todoCheck(req, res, next) {
  const titleSchema = z.string().min(3).max(24);
  const description = z.string().min(3).max(250);
  if (
    titleSchema.safeParse(req.body.title).success &&
    description.safeParse(req.body.description).success
  ) {
    return next();
  }
  return res.json({
    message: "invalid length of title or description",
    value: false,
  });
}
